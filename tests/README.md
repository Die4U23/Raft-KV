...
- `cluster_features_tests` 检查配置命令复制、租约读在窗口内不发探针且仍等待应用、窗口外退回 ReadIndex、去程和回程分开之后租约停在发送时刻、错位应答不刷新联系时间、被移出的投票者安装快照后不再竞选、后加入的非投票者进入法定人数、按地址加入静态列表外的主机、Leader 移除自己，以及 Follower 把成员变更转给 Leader。
