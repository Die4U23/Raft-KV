...
这与 C++ 后端、搜索广告推荐基础架构中的网络接入、配置与元数据存储、服务可靠性和性能诊断有直接关联。比如策略配置需要明确版本，服务重启后需要恢复状态，调用方重试时需要知道上一次操作是否生效。本文项目提供的是这些系统问题的实践基础；版本化策略配置演示仍是后续计划，推荐模型、召回和排序算法也不在当前实现范围内。
...
| 选举、日志复制、多数派提交 | 已实现；单 Raft 组、固定成员配置，默认三节点 | 真实三进程冒烟、短时 TCP 分区、Leader 退出与恢复 | 没有动态成员变更，也没有多分片扩展 |
...
| 快照、日志回收、动态成员变更 | 计划中 | 尚未完成对应实现和验收 | 日志持续增长和落后副本恢复能力仍需完善 |
...
以下均为后续计划。现有构建脚本和基础 KV 能力可以作为起点，但不能把尚未完成的功能写进已实现清单。

| 优先级 | 计划 | 准备解决的问题 | 预期验收方式 |
| --- | --- | --- | --- |
| 1 | **ReadIndex 强一致读** | 旧 Leader 可能返回旧数据；提交进度可能领先于应用进度 | 满足当前任期提交前置条件，确认多数派、建立读屏障并等待应用；覆盖旧 Leader 隔离、应用落后、任期切换和超时 |
| 2 | **版本化策略配置演示** | 把 KV 能力放入一个具体的基础架构使用场景 | 演示配置发布、按版本拉取、本地缓存与回滚；明确缓存新鲜度和失败语义，强一致要求依赖前一项 |
| 3 | **Linux CI 与演示入口** | 自动构建脚本已有，但干净系统复现和持续集成尚未完成 | 从干净 Linux 环境安装依赖、构建、测试；归档失败日志与版本身份，并提供可重复的三节点演示入口 |
| 4 | **请求去重** | 写入生效后回复丢失，客户端向新 Leader 重试可能再次执行 | 设计客户端标识、请求序号、随共识持久化的结果与清理规则；验证跨 Leader 重试不会重复产生业务效果 |
| 5 | **快照与日志回收** | 长期运行日志增长，落后副本仅靠追日志恢复成本较高 | 联合保存状态、应用位置和必要元数据；验证快照安装中断、重启恢复、日志回收后的继续复制 |
...
动态成员变更、其他网络故障、真实存储故障和长期运行验证也仍在待办中，但不混入上述五项已完成成果。后续每完成一项，再附上实现、测试与剩余边界更新本文。
