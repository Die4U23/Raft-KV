...
- 带 `client_id` 和 `request_id` 的 `SET`/`DEL` 只执行一次。`--require_request_id` 默认关闭；关闭时不带序号的写入重试仍会再执行，打开后缺少序号返回 `-ERR request id required`。每个客户端只保留最新序号。`--request_timeout_ms` 默认 0；大于 0 时，已经提案的命令到期后回复一次 `-ERR request timeout; outcome unknown` 并留下日志条目，尚未提案的队列项回复 `-ERR request timeout` 后丢掉。快照按默认 1024 条已应用记录触发。新镜像按 1 MiB 分片键存放，压缩、发送、接收和安装每次处理一块；旧的版本 1 快照元数据仍整份读入。已有同步/异步应用对照保持同步持久化，未比较关闭同步持久化的性能，不能量化该持久化选项的独立成本。
