...
状态：**真实 Linux 诊断 PASS，2026-09-13 完整证据已核验**。[归档结果](../benchmarks/profile-validation.md)：10 万次请求零错误，约 5146 次/秒，客户端约占单核 62.88%，整机空闲约 4.97%。后续[客户端头部读取四轮对照](../benchmarks/client-header-validation.md)也已核验，未观察到收益，保留 classic 默认。下文保留本轮工具的执行方法与统计边界。
...
本轮归档使用 `load_benchmark.py` 的原读取方式及可选测量边界观察回调。后续新增的 `--client-mode` 默认为 classic，合并头部读取须显式选择；负载生成与响应校验逻辑保持一致。回调在吞吐/延迟计时区间外采集 INFO 和进程计数。CPU 窗口包围实际负载，含少量边界 `/proc` 采样开销；不宣称纳秒级同步。
