...
日期：2026-09-09。实现完成、本地检查通过，**新版 Linux 实测已核验**，见[完整证据](../benchmarks/reconnect-validation.md)。此前三组基线测试已经完成，本次针对已测出的重连问题优化；历史证据保留，不将旧结果记为新版本通过。
...
| 新版 Linux 构建 | PASS，源码和测试时二进制记录已核验 |
| Linux `peer_manager_transport_tests` | PASS，9.01 秒；初始拒绝、反复短连接、恢复后收到 Raft 帧、待重连时销毁对象 |
| 新版三节点冒烟和分区 | PASS，冒烟 10 项、分区 5 个阶段；累计拒绝数 11，阈值 100 |
...
实现提交 `ef17ef7` 的完整材料已归档：重连拒绝 24,731 → 11（约减少 99.96%），完整节点日志 5,498,325 → 146,110 字节（约减少 97.34%，两边均含清理）。原有安全性与恢复检查全部通过。

初始 Leader 和选举时序不同，这是单次同类场景前后对比；新版整轮耗时 12.929 秒，旧版 9.593 秒，不能当作故障恢复延迟。退避可能增加恢复等待。本轮不宣称 CPU 或正常业务 QPS 提升，详细身份、窗口、未知请求语义和原始文件见[核验报告](../benchmarks/reconnect-validation.md)。
