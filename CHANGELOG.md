...
现在只在 rpc 序号对得上的应答里更新租约联系时间，记的是这一次 RPC 第一次发出的时刻，只往前移，重试不刷新。对不上的应答不改租约。Leader 自己仍记当前时刻。CheckQuorum 仍用应答到达时刻：往返变长不能把还活着的 Leader 判卸任。复制延迟统计仍用进程单调钟上的 `first_send`，不改成测试钟。

进程内回归把心跳的去程和回程各拨 30 ms：发送后 60 ms 租约仍服务读；再拨到跟随者最早能竞选的时刻，租约不再本地完成，随后重复投递旧应答也不恢复租约。同一段延迟后再过 149 ms，CheckQuorum 仍保持 Leader。可移植 CTest 16/16 通过。`cluster_linearizable.py` 在分区后要等新 Leader 的提交索引越过分区前的值再写入；第一条确认若错过选举超时，该 Leader 会卸任，测试改等下一次已经提交的 Leader。失败时把节点日志末尾打出来。本机这条测试通过。
