...
## 2026-09-09 — 断线重连退避（实现完成，待 Linux 验收）
...
- 新增 Linux 真实 Muduo 生命周期回归，以及分区脚本可选的 `--max-reconnect-refusals` 失败阈值。新版服务构建、真实传输、冒烟和重连次数 **UNRUN**；旧版 PASS 不覆盖本次服务改动。
- 取舍：断线后最多等待 2 秒才发起下一次连接；后续若建连失败，还会进入 Connector 原有最长 30 秒退避，不能承诺两秒内恢复。
- 未改同步持久化、多数派确认或 GET 一致性语义；尚未宣称 QPS、CPU 或重连次数改善。

原因、源码依据和执行方法见[重连优化记录](docs/optimizations/peer-reconnect-backoff.md)。收到真实报告后在本条补充新旧计数、日志大小、测试提交和证据链接。
