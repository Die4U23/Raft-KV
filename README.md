...
进度标签是 `v0.3.0`（快照、去重、配置、成员变更、多分片、认证、租约读）。`v0.2.0` 只到 Pre-Vote。`v0.3.0` 不在 `main`。
...
- `--request_timeout_ms=N`：写入等待超过 N 毫秒就回复。0 等到应用或卸任（默认）。已经进入日志的命令回复 `-ERR request timeout; outcome unknown` 并留在日志里；还在写队列里的命令直接丢掉，回复 `-ERR request timeout`
...
- `--cluster_token` 为空时 Raft 帧不变。非空时帧外是 `MAC1`、内层长度和 HMAC-SHA256，校验失败就关闭连接。`--client_token` 为空时客户端命令不认证。`--lease_reads` 默认关闭。打开后，远程投票者的租约联系时间是已确认 AppendEntries 或 InstallSnapshot 的发送时刻。多数派里第 quorum 新的那一次加上（150 ms − 10 ms）之前，Leader 把读排到当前提交位置，仍要等应用到那一位；恰好到达该窗口或窗口之外退回 ReadIndex。回程延迟不再延长这段租约。CheckQuorum 仍按应答到达时刻计算，往返变长不会因此卸任。跟随者的时钟如果在一个选举超时里快过这 10 ms，仍可能在 Leader 认为租约有效时开始竞选。
