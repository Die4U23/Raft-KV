...
- 已应用条目超过快照距离（默认 1024）后压缩日志，并用单条 InstallSnapshot 追平落后副本。镜像超过 8 MiB 时跳过压缩、保留日志。快照不分片。
...
3. ✅ ~~快照 / InstallSnapshot / 日志压缩~~：应用后按距离导出 KV 镜像并截断日志前缀。落后副本收到一条 InstallSnapshot。日志快照先于 KV 落盘，重启时用日志里的镜像补上尚未安装的状态。
